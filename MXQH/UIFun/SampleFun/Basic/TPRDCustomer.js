...
angular.module('app')
.controller('TPRDCustomerCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function GetCondition() {
        var li = [];
        li.push({ name: "TypeCode", value: "RDCustomer" });
        li.push({ name: "TypeName", value: "研发客户" });
        if (vm.Ser.Code) {
    ...
    function Delete(item) {
        ...
        AjaxService.GetPlansTop("RDReceivement", { name: "CustomerID", value: item.ID }).then(function (data) {
            if (data.length == 0) {
