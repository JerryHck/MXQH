...
angular.module('app')
.controller('TPRDDeptCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function GetCondition() {
        var li = [];
        li.push({ name: "TypeCode", value: "RDDept" });
        li.push({ name: "TypeName", value: "研发部门" });
        if (vm.Ser.Code) {
    ...
    function Delete(item) {
        ...
        AjaxService.GetPlansTop("RDReceivement", { name: "ReturnDeptID", value: item.ID }).then(function (data) {
            if (data.length == 0) {
