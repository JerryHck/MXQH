...
angular.module('app')
.controller('BCProductTypeCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function GetCondition() {
        var li = [];
        li.push({ name: "TypeCode", value: "BCProductType" });
        li.push({ name: "TypeName", value: "商务-型号" });
        if (vm.Ser.Code) {
    ...
    function Delete(item) {
        ...
        AjaxService.GetPlansTop("BCRcvDetail", { name: "TypeID", value: item.ID }).then(function (data) {
            if (data.length == 0) {
                ...
                AjaxService.GetPlansTop("BCReceivement", { name: "ID", value: item.RcvID }).then(function (data) {
                    toastr.error("存在该产品类型入库单[" + data[0].DocNo + "]，不能删除！");
                });
            }
        });
    }
