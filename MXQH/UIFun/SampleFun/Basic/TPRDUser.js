...
angular.module('app')
.controller('TPRDUserCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function GetCondition() {
        var li = [];
        li.push({ name: "TypeCode", value: "RDUser" });
        li.push({ name: "TypeName", value: "研发用户" });
        if (vm.Ser.Code) {
    ...
    function Delete(item) {
        ...
        AjaxService.GetPlansTop("RDReceivement", { name: "Borrower", value: item.Name }).then(function (data) {
            if (data.length == 0) {
                ...
                console.log(data);
                toastr.error("存在该领借人入库单[" + data[0].DocNo + "]，不能删除！");
            }
        });
    }
