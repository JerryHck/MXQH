...
angular.module('app')
.controller('TPPCDeptCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function GetCondition() {
        var li = [];
        li.push({ name: "TypeCode", value: "PCDept" });
        li.push({ name: "TypeName", value: "产品中心部门" });
        if (vm.Ser.Code) {
    ...
    function Delete(item) {
        ...
        AjaxService.GetPlansTop("PCReceivement", { name: "ReturnDeptID", value: item.ID }).then(function (data) {
            if (data.length == 0) {
                ...
                console.log(data);
                toastr.error("存在该部门入库单[" + data[0].DocNo + "]，不能删除！");
            }
        });
    }
