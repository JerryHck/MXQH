'use strict';
angular.module('app')
.controller('PCRcvCtrl', ['$rootScope', '$scope', 'Dialog', 'toastr', 'AjaxService', 'Form', '$window',
function ($rootScope, $scope, Dialog, toastr, AjaxService, Form, $window) {
    ...
    //绑定数据
    function DataBind() {
        vm.promise = AjaxService.ExecPlan("PCReceivement", "GetList", vm.page).then(function (data) {
            vm.List = data.data;
    ...
    function Add() {
        ...
        AjaxService.ExecPlan("SerialNumberSet", "preview", en).then(function (data) {
            ...
            //PK生成设定
            var snList = [{ name: "PCRcv", col: "DocNo", parm: "DocNo" }];
            //SN生产
    ...
    function SaveEdit(index) {
        ...
        vm.promise = AjaxService.PlanUpdate("PCReceivement", en).then(function (data) {
            DataBind();
            toastr.success('更新成功');
        });
    }
    ...
    function Open(resolve) {
        Dialog.open("PCRcvDetail", resolve).then(function (data) {
            DataBind();
    ...
    function Delete(item) {
        vm.promise = AjaxService.ExecPlan("PCReceivement", "Delete", { ID: item.ID }).then(function (data) {
