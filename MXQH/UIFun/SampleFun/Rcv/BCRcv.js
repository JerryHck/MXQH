'use strict';
angular.module('app')
.controller('BCRcvCtrl', ['$rootScope', '$scope', 'Dialog', 'toastr', 'AjaxService', 'Form', '$window',
function ($rootScope, $scope, Dialog, toastr, AjaxService, Form, $window) {
    ...
    //绑定数据
    function DataBind() {
        vm.promise = AjaxService.ExecPlan("BCReceivement", "GetList", vm.page).then(function (data) {
            vm.List = data.data;
    ...
    function Add() {
        ...
        AjaxService.ExecPlan("SerialNumberSet", "preview", en).then(function (data) {
            ...
            //PK生成设定
            var snList = [{ name: "BCRcv", col: "DocNo", parm: "DocNo" }];
            //SN生产
    ...
    function SaveInsert() {
        ...
        vm.promise = AjaxService.PlanInsert("BCReceivement", vm.NewItem).then(function (data) {
            DataBind();
            toastr.success('新增成功');
            vm.IsInsert = false;
        });
    ...
    function SaveEdit(index) {
        ...
        vm.promise = AjaxService.PlanUpdate("BCReceivement", en).then(function (data) {
            DataBind();
            toastr.success('更新成功');
        });
    }
    ...
    function Open(resolve) {
        Dialog.open("BCRcvDetail", resolve).then(function (data) {
            DataBind();
    ...
    function Delete(item) {
        vm.promise = AjaxService.ExecPlan("BCReceivement", "Delete", { ID: item.ID }).then(function (data) {
        ...
        //vm.promise = AjaxService.PlanDelete("RDReceivement", en).then(function (data) {
        //    DataBind();
        //    toastr.success('删除成功');
        //});
    }
