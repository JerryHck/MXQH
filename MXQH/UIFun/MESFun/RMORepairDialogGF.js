'use strict';
angular.module('app').controller('RMORepairGFctrl', RMORepairGFctrl);

RMORepairGFctrl.$inject = ['$scope', '$uibModalInstance', 'Dialog', 'Form', 'ItemData', 'toastr', 'AjaxService'];

function RMORepairGFctrl($scope, $uibModalInstance, Dialog, Form, ItemData, toastr, AjaxService) {
    var vm = this;
    ...
    function SelectRMO(e) {
        ...
        if (keycode == 13 && vm.SNCode) {
            var bc = {};
            bc.SNCode = vm.SNCode;
            if (bc.SNCode == null || bc.SNCode == '') {
                toastr.error('内控码不能为空！');
            } else if (bc.SNCode != undefined) {
                AjaxService.GetPlan("SelectProRepairGF", [{ name: "SNCode", value: bc.SNCode }]).then(function (data) {
                    vm.item = data;
                    if (vm.item.SNCode == null) {
    ...
    function Alter() {
        ...
        vm.promise = AjaxService.ExecPlan("SelectProRepairGF", "updateGF", en).then(function (data) {
            console.log(data);
