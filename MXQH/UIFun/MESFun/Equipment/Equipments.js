'use strict';
angular.module('app')
.controller('EquipmentsCtrl', ['$rootScope', '$scope', 'Dialog', 'toastr', 'AjaxService', 'Form',
function ($rootScope, $scope, Dialog, toastr, AjaxService, Form) {
    ...
    function DataBind() {
        GetCondition();
        vm.promise = AjaxService.ExecPlan("Equipment", "GetList", vm.page).then(function (data) {
            vm.List = data.data;
    ...
    function Open(resolve) {
        Dialog.open("EquipmentDialog", resolve).then(function (data) {
            DataBind();
