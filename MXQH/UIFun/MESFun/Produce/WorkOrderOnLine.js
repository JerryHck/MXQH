...
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    function CheckBindSn() {
        if (vm.RoutingData.boProcedureID != 604) return;
    ...
    function Save() {
        ...
        //console.log(en)
        var dataCheck = AjaxService.ExecPlanWait("MESPlanExMain", "check", en);
        if (dataCheck.data && dataCheck.data[0].MsgType == 'Success') {
            var enSave = dataCheck.data1[0];
            //enSave.SNCode = "";
            enSave.IMEICode = "";
            if (vm.RoutingData.boProcedureID == 604 && !enSave.SNCode) {
            ...
            //console.log(enSave);
            //同步执行方式
            var data = AjaxService.ExecPlanWait("MesMxWOrder", "save", enSave);
            if (data.data && data.data[0].MsgType == 'Success') {
                vm.OrderCount = dataCheck.data2[0];
                vm.MesList.splice(0, 0, { Id: vm.MesList.length + 1, IsOk: true, Msg: data.data[0].Msg });
                AjaxService.PlayVoice('success.mp3');
                //打印
                if (vm.RoutingData.boProcedureID == 604) {
                    //获取打印数据
