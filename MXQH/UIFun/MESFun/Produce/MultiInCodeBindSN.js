...
angular.module('app')
.controller('MultiInCodeBindSNCtrl', ['$scope', '$http', 'AjaxService', 'toastr', '$window', 'MyPop',
function ($scope, $http, AjaxService, toastr, $window, MyPop) {
    ...
    function GetOrder() {
        ...
        AjaxService.ExecPlan("BindCode", "getOrder", en, false).then(function (data) {
            ...
            if (!data.data[0] || !data.data[0].WorkOrder) {
                ...
                vm.NoList = [];
                for (var i = 0, len2 = vm.OrderData.BSN_SN; i < len2; i++) {
                    vm.NoList.push(i + 1);
                }
    ...
    function Action() {
        vm.BSNList = vm.BSNList || [];
        vm.KeySn = undefined;
        vm.CharName = undefined;
        var en = { InternalCode: vm.KeyInCode, WorkOrder: vm.Item.WorkOrder, TbName: vm.OrderData.TbName, ClName: vm.OrderData.ClName, IsLock: vm.Item.IsLock, ProType: 1 };
        var list = []
        for (var i = 0, len = vm.BSNList.length; i < len; i++) {
            list.push({ InternalCode: vm.BSNList[i].InternalCode });
        }
        en.InCodeList = JSON.stringify(vm.BSNList);
        en.TempColumns = "InCodeList";
        vm.promise = AjaxService.ExecPlan("BindCode", "packCheck", en, true).then(function (data) {
            //console.log(data)
            if (data.data[0].MsgType == "Error") {
                ...
                vm.BSNList.push({ InternalCode: data.data1[0].InternalCode });
                vm.ThisSnCode = vm.NewBind.InternalCode;
                vm.NewBind.InternalCode = undefined;
                if (vm.BSNList.length < vm.NoList.length) {
                    //var mss = "内控码[" + KeyInCode + ']  SN码 [' + data.data1[0].SNCode + '] 绑定成功';
                    var Msg = { Id: vm.MesList.length + 1, IsOk: true, Msg: "扫描成功" };
                    vm.MesList.splice(0, 0, Msg);
                    AjaxService.PlayVoice('success.mp3');
                    vm.isFinist = true;
                }
                else if (vm.BSNList.length == vm.NoList.length) {
                    //生成SN
                    SaveBindCode();
                }
    ...
    function SaveBindCode() {
        ...
        //内部虚拟码生成
        SNList.push({ name: "VerSNCode", col: "VSNCode", charName: "", multi: true, count: count });
        vm.ThisBind.SNColumns = JSON.stringify(SNList);
        //BSN列表添加
        vm.ThisBind.BSNList = JSON.stringify(vm.BSNList);
        vm.ThisBind.TempColumns = "BSNList";

        vm.promise = AjaxService.ExecPlan("BindCode", 'multiSNSave', vm.ThisBind).then(function (data) {
            //console.log(data)
            vm.isFinist = true;
            vm.BSNList = [];
            if (data.data[0].MsgType == "Error") {
                ...
                //获取打印数据
                var en = { SNCode: data.data1[0].VirSNCode };
