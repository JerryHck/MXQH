...
angular.module('app')
.controller('MesBasicRouteCtrl', ['$rootScope', '$scope', 'MyPop', 'AjaxService', 'toastr', '$window',
function ($rootScope, $scope, MyPop, AjaxService, toastr, $window) {
    ...
    vm.GetRouterList = GetRouterList;
    ...
    function GetRouterList() {
        var en = { name: "IsUse", value: vm.IsUse }
        vm.promise = AjaxService.GetPlans("MESboRoute", en).then(function (data) {
    ...
    function AddRoute() {
        vm.NewRoute = {
            ID: -1, RouteName: "新路由工艺", IsUse: true, Procedure: [],
        };
        vm.editRoute = true;
    ...
    //删除工艺
    function Delete(Id) {
        vm.promise = AjaxService.ExecPlan("MESboRoute", "delete", { ID: Id }).then(function (data) {
            if (data.data[0].MsgType == "Error") {
                toastr.error(data.data[0].MsgText);
            }
            else if (data.data[0].MsgType == "Success") {
                toastr.success("删除成功");
                vm.SelectedRo = undefined;
                GetRouterList();
            }
        })
    }
    ...
    function isExists() {
        if (vm.SelectedRo.RoutingName) {
            var en = [{ name: "RouteName", value: vm.SelectedRo.RoutingName },
                { name: "ID", value: vm.SelectedRo.ID, type: "!=" }];
            AjaxService.GetPlan('BoRoute', en).then(function (data) {
