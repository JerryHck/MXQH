...
function SysUISelectDialogCtrl($rootScope, $scope, $uibModalInstance, Form, ItemData, toastr, AjaxService, $window) {
    ...
    function ToHtml() {
        ...
        html += '        <ui-select-choices repeat="' + (data.ReturnColumn && data.ReturnColumn != '' ? 'item.' + data.ReturnColumn + ' as ' : "") + 'item in data | filter: $select.search  | limitTo:50"  refresh="refresh($select.search)">\n';
