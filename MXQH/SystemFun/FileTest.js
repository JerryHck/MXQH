...
function ($scope, serviceUrl, $window, AjaxService, FileService, toastr, FileUrl, $cookieStore, $state) {
    ...
    vm.promise = AjaxService.CallDll('Auctus.AESEncoder', 'Auctus.AESEncoder', 'AesEncrypt', { str: "dasd" }).then(function (data) {
        console.log(data)
        AjaxService.CallDll('Auctus.AESEncoder', 'Auctus.AESEncoder', 'AesDecrypt', { str: data }).then(function (data2) {
            console.log(data2)
        });
    });
