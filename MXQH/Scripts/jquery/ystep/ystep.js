...
(function($){
  $.fn.extend({
    ...
    loadStep: function(params){
      ...
      var baseHtml =  "<div class='ystep-container'>"+
                        ...
                        "<div class='ystep-progress'>"+
                          "<p class='ystep-progress-bar'>"+
                            "<div class='ystep-progress-container'><span class='ystep-progress-highlight' style='width:0%'></span>"+
							"<span class='ystep-progress-breakline'></span></div>" +
                          "</p>"+
                        "</div>"+
                      "</div>";
      //步骤框架
      ...
      var logic = {
        size: {
          small: function($html){
			...
			$html.find(".ystep-progress-breakline").css({
              width: '0%',
			  left: '0%'
            });
      ...
      $(this).each(function(i,n){
        ...
        for(i=0;i<arrayLength;i++){
		  ...
		  $stepHtml.attr("data-html",_s.html);
		  $stepHtml.attr("data-trigger",_s.trigger);
		  $stepHtml.attr("data-delay",_s.delay);
    ...
    //跳转到指定步骤
    setStep: function(step,breakpoint) {
		var _bpoint = breakpoint || -1;
      $(this).each(function(i,n){
        //获取当前容器下所有的步骤
        var $steps = $(n).find(".ystep-container").find("li");
        var $progress =$(n).find(".ystep-container").find(".ystep-progress-highlight");
		var $breakline =$(n).find(".ystep-container").find(".ystep-progress-breakline");
        //判断当前步骤是否在范围内
        if(1<=step && step<=$steps.length){
		  ...
		  var breakpoint_len = _bpoint == -1 ? 0 : _bpoint;
		  break_scale = Math.round((breakpoint_len-step)*100/($steps.length-1))+break_scale;
          ...
          $progress.animate({
            ...
            speed: 1000,
              ...
              $steps.each(function(j,m){
                var _$m = $(m);
                var _j = j+1;
				if(_bpoint != -1 && _j < (_bpoint + 1) && _j > step){
					_$m.attr("class","ystep-step-break");
				}
                else if(_j < step){
