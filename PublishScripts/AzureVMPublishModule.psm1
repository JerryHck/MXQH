#  AzureVMPublishModule.psm1 是一个 Windows PowerShell 脚本模块。此模块导出可自动执行 Web 应用程序的生命周期管理的 Windows PowerShell 函数。你可按原样使用这些函数，也可以针对你的应用程序和发布环境自定义这些函数。
...
.PARAMETER  Message
指定在错误消息调用过程中使用的消息。你可以通过管道将消息字符串传送给此函数。
...
.PARAMETER  Message
指定在详细消息调用过程中使用的消息。你可以通过管道将消息字符串传送给此函数。
...
<#
.SYNOPSIS
写入一条添加了当前时间作为前缀的主机消息。

.DESCRIPTION
此函数向主机程序(Write-Host)写入一条添加了当前时间作为前缀的消息。写入到主机程序所产生的影响不尽相同。大多数托管 Windows PowerShell 的程序都会将这些消息写入到标准输出。

.PARAMETER  Message
指定不带日期的基础消息。你可以通过管道将消息字符串传送给此函数。
...
.PARAMETER  Object
指定在测试过程中使用的对象。请输入一个包含对象或者包含可返回对象的表达式的变量。你不能指定类型(例如 [DateTime])，也不能通过管道向此函数传送对象。
...
.LINK
PSModuleInfo object (https://msdn.microsoft.com/en-us/library/system.management.automation.psmoduleinfo(v=vs.85).aspx)
#>
...
<#
.SYNOPSIS
在当前订阅中查找名为 "devtest*" 的 Microsoft Azure 存储帐户。

.DESCRIPTION
Get-AzureVMStorage 函数返回指定位置或关联组内第一个采用 "devtest*" (不区分大小写)名称模式的存储帐户的名称。如果 "devtest*" 存储帐户与位置或关联组不匹配，此函数将会忽略该存储帐户。你必须指定位置或关联组。

.PARAMETER  Location
指定存储帐户的位置。有效的值为 Microsoft Azure 位置，例如“West US”。你可以输入位置，也可输入关联组，但不能同时输入这两者。

.PARAMETER  AffinityGroup
指定存储帐户的关联组。你可以输入位置，也可输入关联组，但不能同时输入这两者。
...
<#
.SYNOPSIS
使用以 "devtest" 开头的唯一名称创建一个新的 Microsoft Azure 存储帐户。

.DESCRIPTION
Add-AzureVMStorage 函数在当前订阅中创建一个新的 Microsoft Azure 存储帐户。该帐户的名称以 "devtest" 开头，后跟一个唯一的字母数字字符串。此函数返回新存储帐户的名称。你必须为新存储帐户指定位置或关联组。
...
function Add-AzureVMStorage
{
    ...
    # 使用位置/关联组创建一个新的 Microsoft Azure 存储帐户
...
指定你 Web 项目的 JSON 配置文件的路径及名称。Visual Studio 会在你创建 Web 项目时自动生成 JSON 文件，并将该文件存储在你解决方案中的 PublishScripts 文件夹中。

.PARAMETER HasWebDeployPackage
指示 Web 应用程序有一个 Web 部署包 ZIP 文件。若要指定 $true 值，请使用 -HasWebDeployPackage 或 HasWebDeployPackage:$true。若要指定 false 值，请使用 HasWebDeployPackage:$false。此参数是必需的。
...
.PARAMETER  VM
指定虚拟机对象。请输入一个 VM 对象，例如 New-AzureVM 或 Get-AzureVM cmdlet 返回的类型。你可以通过管道将 Get-AzureVM 返回的对象传送给 Add-AzureVMEndpoints。

.PARAMETER  Endpoints
指定由要向 VM 添加的终结点组成的数组。通常，这些终结点来自于 Visual Studio 为 Web 项目生成的 JSON 配置文件。请使用此模块中的 Read-ConfigFile 函数将该文件转换成哈希表。这些终结点是该哈希表的 cloudservice 键的一个属性($<hashtable>.cloudservice.virtualmachine.endpoints)。例如，
...
<#
.SYNOPSIS
在 Microsoft Azure 订阅中为新虚拟机创建所有元素。

.DESCRIPTION
此函数会创建一个 Microsoft Azure 虚拟机(VM)，并返回部署后的 VM 的 URL。此函数会设置先决条件，然后调用 New-AzureVM cmdlet (Azure 模块)来创建一个新 VM。
-- 此函数通过调用 New-AzureVMConfig cmdlet (Azure 模块)来获取虚拟机配置对象。
-- 如果你包含 Subnet 参数以便将 VM 添加到某个 Azure 子网，则此函数会调用 Set-AzureSubnet 来为 VM 设置子网列表。
-- 此函数通过调用 Add-AzureProvisioningConfig (Azure 模块)来向 VM 配置添加元素。它使用管理员帐户和密码创建一个独立的 Windows 设置配置(-Windows)。
-- 此函数调用此模块中的 Add-AzureVMEndpoints 函数来添加由 Endpoints 参数指定的终结点。此函数接受 VM 对象作为参数并返回包含所添加终结点的 VM 对象。
-- 此函数调用 Add-AzureVM cmdlet 来创建一个新的 Microsoft Azure 虚拟机，并返回该新 VM。函数参数的值通常从 Visual Studio 为 Microsoft Azure 集成的 Web 项目生成的 JSON 配置文件中获取。此模块中的 Read-ConfigFile 函数可将 JSON 文件转换成哈希表。请将该哈希表的 cloudservice 键保存在一个变量中(作为 PSCustomObject)，然后使用该自定义对象的属性作为参数值。

.PARAMETER  VMName
为新 VM 指定一个名称。该 VM 名称在云服务中必须是唯一的。此参数是必需的。

.PARAMETER  VMSize
指定 VM 的大小。有效的值有:“ExtraSmall”、“Small”、“Medium”、“Large”、“ExtraLarge”、“A5”、“A6”和“A7”。此值以 New-AzureVMConfig 的 InstanceSize 参数值的形式提交。此参数是必需的。

.PARAMETER  ServiceName
指定现有的 Microsoft Azure 服务或指定新 Microsoft Azure 服务的名称。此值提交给 New-AzureVM cmdlet 的 ServiceName 参数，由该 cmdlet 将新虚拟机添加到现有的 Microsoft Azure 服务；或者，如果指定了 Location 或 AffinityGroup，该 cmdlet 会在当前订阅中创建一个新的虚拟机及服务。此参数是必需的。

.PARAMETER  ImageName
为操作系统磁盘指定要使用的虚拟机映像的名称。此参数以 New-AzureVMConfig cmdlet 的 ImageName 参数值的形式提交。此参数是必需的。

.PARAMETER  UserName
指定管理员用户名。此用户名以 Add-AzureProvisioningConfig 的 AdminUserName 参数值的形式提交。此参数是必需的。

.PARAMETER  UserPassword
指定管理员用户帐户的密码。此密码以 Add-AzureProvisioningConfig 的 Password 参数值的形式提交。此参数是必需的。

.PARAMETER  Endpoints
指定由要向 VM 添加的终结点组成的数组。此值提交给此模块导出的 Add-AzureVMEndpoints 函数。此参数是可选的。通常，这些终结点来自于 Visual Studio 为 Web 项目生成的 JSON 配置文件。请使用此模块中的 Read-ConfigFile 函数将该文件转换成哈希表。这些终结点是该哈希表的 cloudService 键的一个属性($<hashtable>.cloudservice.virtualmachine.endpoints)。

.PARAMETER  AvailabilitySetName
为新 VM 指定可用性集的名称。当你将多个虚拟机放入一个可用性集内时，Microsoft Azure 会设法将这些虚拟机分别放在不同的主机上，以便提高在其中一个虚拟机发生故障时服务的连续性。此参数是可选的。

.PARAMETER  VNetName
指定将新虚拟机部署到的虚拟网络的名称。此值提交给 Add-AzureVM cmdlet 的 VNetName 参数。此参数是可选的。

.PARAMETER  Location
为新 VM 指定一个位置。有效的值为 Microsoft Azure 位置，例如“West US”。默认值为订阅的位置。此参数是可选的。

.PARAMETER  AffinityGroup
为新 VM 指定一个关联组。关联组是一组相关的资源。当你指定了关联组时，Microsoft Azure 会设法将该组中的资源放在一起，以提升效率。

.PARAMETER  Subnet
指定新 VM 配置的子网。此值提交给 Set-AzureSubnet cmdlet (Azure 模块)，该 cmdlet 接受一个 VM 和一个子网名称数组作为参数，并返回一个在配置中采用这些子网的 VM。

.PARAMETER EnableWebDeployExtension
为 VM 做好部署准备。为 VM 做好部署准备。此参数是可选的。如果未指定此参数，则会创建 VM，但不会部署它。此参数的值包含在 Visual Studio 为云服务生成的 JSON 配置文件中。

.PARAMETER VMImage
指定 ImageName 为 VMImage 而非 OSImage 的名称。此参数是可选的。如果未指定此参数，则将 ImageName 视为 OSImage。此参数的值包含在 Visual Studio 为虚拟机生成的 JSON 配置文件中。

.PARAMETER GeneralizedImage
对于 VMImage，请指定 OS 状态是否为通用。此参数是可选的。如果未指定此参数，则脚本的行为将与专用 VMImage 类似。对于 OSImage，请忽略此参数。此参数的值包含在 Visual Studio 为虚拟机生成的 JSON 配置文件中。
...
.EXAMPLE
 此命令调用 Add-AzureVM 函数。其很多参数值都来自于 $CloudServiceConfiguration 对象。此 PSCustomObject 是 Read-ConfigFile 函数返回的哈希表的 cloudservice 键和值。来源是 Visual Studio 为 Web 项目生成的 JSON 配置文件。
...
function Add-AzureVM
{
    ...
    # 创建一个新的 Microsoft Azure VM 配置对象。
    ...
    if (!$vm)
    {
        throw 'Add-AzureVM: 未能创建 Azure VM 配置。'
    }
    ...
    # VMImages 目前不支持该位置，将在映像存在的相同存储帐户(位置)中创建新 VM
...
<#
.SYNOPSIS
获取指定的 Microsoft Azure 虚拟机。

.DESCRIPTION
Find-AzureVM 函数根据服务名称和虚拟机名称获取 Microsoft Azure 虚拟机(VM)。此函数通过调用 Test-AzureName cmdlet (Azure 模块)来验证 Microsoft Azure 中是否存在该服务名称。如果存在，此函数会调用 Get-AzureVM cmdlet 来获取相应 VM。此函数返回一个包含 vm 和 foundService 键的哈希表。
-- FoundService: 如果 Test-AzureName 找到了相应服务，则为 $True； 否则为 $False
-- VM: 当 FoundService 为 True 并且 Get-AzureVM 返回 VM 对象时，包含相应的 VM 对象。

.PARAMETER  ServiceName
现有 Microsoft Azure 服务的名称。此参数是必需的。
...
InstanceSize          : Small
AvailabilitySetName   :
DNSName               : https://contoso.cloudapp.net/
ServiceName           : Contoso
...
.DESCRIPTION
New-AzureVMEnvironment 函数会在订阅中查找或创建一个与 Visual Studio 为 Web 项目生成的 JSON 配置文件中的值匹配的虚拟机。它接受作为 Read-ConfigFile 返回的哈希表的 cloudservice 键的 PSCustomObject。此数据来自于 Visual Studio 生成的 JSON 配置文件。此函数会在订阅中查找服务名称和虚拟机名称与 CloudServiceConfiguration 自定义对象中的值匹配的虚拟机(VM)。如果此函数找不到匹配的 VM，它会调用此模块中的 Add-AzureVM 函数，并使用 CloudServiceConfiguration 对象中的值来创建 VM。虚拟机环境包含一个名称以 "devtest" 开头的存储帐户。如果此函数在订阅中找不到采用该名称模式的存储帐户，它会创建一个这样的存储帐户。此函数返回一个包含 VMUrl、userName 和 Password 键及字符串值的哈希表。

.PARAMETER  CloudServiceConfiguration
接受 PSCustomObject，它包含 Read-ConfigFile 函数返回的哈希表的 cloudservice 属性。所有值均来自于 Visual Studio 为 Web 项目生成的 JSON 配置文件。你可以在你解决方案中的 PublishScripts 文件夹中找到该文件。此参数是必需的。
$config = Read-ConfigFile -ConfigurationFile <file>.json $cloudServiceConfiguration = $config.cloudService

.PARAMETER  VMPassword
接受一个包含 Name 和 Password 键的哈希表，如: @{Name = "admin"; Password = "password"} 此参数是可选的。如果你忽略此参数，则默认值为 JSON 配置文件中的虚拟机用户名和密码。
...
<#
.SYNOPSIS
向 Microsoft Azure 部署 Web 包。

.DESCRIPTION
Publish-WebPackage 函数使用 MsDeploy.exe 和一个 Web 部署包 ZIP 文件将资源部署到 Microsoft Azure 网站。此函数不生成任何输出。如果调用 MSDeploy.exe 失败，此函数将引发异常。如果想获取更详细的输出，可使用 Verbose 通用参数。
...
允许到 Web 部署终结点的不受信任的 SSL 连接。在调用 MSDeploy.exe 时使用此参数。此参数是可选的。

.PARAMETER ConnectionString
指定 SQL 数据库的连接字符串。此参数接受包含 Name 和 ConnectionString 键的哈希表。Name 的值是数据库的名称。ConnectionString 的值是 JSON 配置文件中的 connectionStringName。
...
https://go.microsoft.com/fwlink/?LinkId=391354
#>
...
<#
.SYNOPSIS
向 Microsoft Azure 部署虚拟机。

.DESCRIPTION
Publish-WebPackageToVM 函数是一个帮助程序函数，因为该函数会验证参数值，然后调用 Publish-WebPackage 函数。

.PARAMETER  VMDnsName
指定 Microsoft Azure 虚拟机的 DNS 名称。此参数是必需的。

.PARAMETER IisWebApplicationName
为虚拟机指定 IIS Web 应用程序的名称。此参数是必需的。这是你的 Visual Studio Web 应用程序的名称。你可以在 Visual Studio 生成的 JSON 配置文件的 webDeployparameters 特性中找到此名称。
...
function Publish-WebPackageToVM
{
    ...
    if (!$VMDnsUrl)
    {
        throw ('Publish-WebPackageToVM: 无效的 URL' + $VMDnsUrl)
    }
...
<#
.SYNOPSIS
创建一个供你连接到 Microsoft Azure SQL 数据库的字符串。
...
接受由 PSCustomObjects 组成的数组，这些对象来自于当 JSON 文件有网站属性时 Read-ConfigFile 函数返回的 JSON 文件。它包含 environmentSettings.databases 属性。你可以通过管道将该列表传送给此函数。
PS C:\> $config = Read-ConfigFile <name>.json
...
为 SQL 数据库服务器管理员指定密码。输入一个包含 Name 和 Password 键的哈希表。Name 值为 SQL 数据库服务器的名称。Password 值为管理员密码。例如: @Name = "TestDB1"; Password = "password" 此参数是可选的。如果你省略此参数或者 SQL 数据库服务器名称与 $DatabaseConfig 对象的 serverName 属性的值不匹配，则此函数将在连接字符串中对 SQL 数据库使用 $DatabaseConfig 对象的 Password 属性。
...
.PARAMETER DatabaseConfig
接受 PSCustomObject，它来自于当 JSON 文件有网站属性时 Read-ConfigFile 函数返回的 JSON 配置文件。它包含 environmentSettings.databases 属性。你不能通过管道将该对象传送给此函数。Visual Studio 会为所有 Web 项目都生成一个 JSON 配置文件，并将该文件存储在你的解决方案的 PublishScripts 文件夹中。
